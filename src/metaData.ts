...
type IProvider<T> = (type: string, imageId: string) => T;
...
const providers: Array<IMetadataProvider> = [];
 ...
 * @memberof Metadata
 */
export function addProvider<T> (provider: IProvider<T>, priority = 0) {
  let i;
 ...
 * @memberof Metadata
 */
export function removeProvider (provider: IProvider<any>) {
 ...
 * @memberof Metadata
 */
function getMetaData<T> (type: string, imageId: string) {
