...
import generateColorLut from '../internal/generateColorLut';
import storedColorPixelDataToCanvasImageData from '../internal/storedColorPixelDataToCanvasImageData';
import storedRGBAPixelDataToCanvasImageData from '../internal/storedRGBAPixelDataToCanvasImageData';
 ...
 * @memberof rendering
 */
function getLut (image: Image, viewport: Viewport) {
  if (!viewport.voi) {
    throw new Error('voi must be specified in the viewport');
  }
 ...
 * @memberof rendering
 */
function getRenderCanvas (enabledElement: EnabledElement | EnabledElementLayer, image: Image, invalidated: boolean = false) {
 ...
 * @memberof rendering
 */
export function renderColorImage (enabledElement: EnabledElement, invalidated: boolean = false) {
  ...
  if (!context) {
    throw new Error('renderColorImage: could not get canvas context');
  }
  ...
  if (!enabledElement.viewport) {
    throw new Error('renderColorImage: viewport must be specified');
  }
...
export function addColorLayer (layer: EnabledElementLayer, invalidated: boolean = false) {
  if (layer === undefined) {
  ...
  if (!layer.canvas) {
    throw new Error('addColorLayer: layer.canvas must be defined');
  }
  const context = layer.canvas.getContext('2d');

  if (!context) {
    throw new Error('addColorLayer: could not get canvas context');
  }

  if (!layer.viewport) {
    throw new Error('addColorLayer: viewport must be specified');
  }
  // Turn off image smooth/interpolation if pixelReplication is set in the viewport
