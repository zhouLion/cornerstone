 ...
 * @memberof rendering
 */
export default function (image, viewport, invalidated = false) {
  if (!viewport) {
  ...
  generateLut(image, viewport.voi.windowWidth, viewport.voi.windowCenter, Boolean(viewport.invert), viewport.modalityLUT, viewport.voiLUT);
  image.cachedLut.windowWidth = viewport.voi.windowWidth;
  image.cachedLut.windowCenter = viewport.voi.windowCenter;
  image.cachedLut.invert = viewport.invert;
