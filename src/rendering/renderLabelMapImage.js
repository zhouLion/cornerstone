...
function getRenderCanvas (enabledElement, image, invalidated = false) {
  ...
  if (!enabledElement.viewport) {
    throw new Error('viewport must be set to render an image');
  }
 ...
 * @memberof rendering
 */
export function renderLabelMapImage (enabledElement, invalidated = false) {
  ...
  if (!enabledElement.canvas) {
    throw new Error('renderLabelMapImage: enabledElement must have a canvas');
  }
  // Get the canvas context and reset the transform
  const context = enabledElement.canvas.getContext('2d');

  if (!context) {
    throw new Error('renderLabelMapImage: could not get canvas context');
  }
 ...
 * @returns {void}
 */
export function addLabelMapLayer (layer, invalidated = false) {
  if (layer === undefined) {
  ...
  if (!context) {
    throw new Error('Could not getContext');
  }
  if (!layer.viewport) {
    throw new Error('viewport must be set to render an image');
  }
