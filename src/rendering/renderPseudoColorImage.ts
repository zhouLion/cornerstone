 ...
 * @memberof rendering
 */
function getRenderCanvas (enabledElement: EnabledElement | EnabledElementLayer, image: Image, invalidated = false) {
  ...
  if (!enabledElement.viewport) {
    throw new Error('viewport must be set to render image');
  }
 ...
 * @memberof rendering
 */
export function renderPseudoColorImage (enabledElement: EnabledElement, invalidated = false) {
  ...
  if (!enabledElement.viewport) {
    throw new Error('drawImage: viewport must be set to draw image');
  }
 ...
 * @returns {void}
 */
export function addPseudoColorLayer (layer: EnabledElementLayer, invalidated = false) {
  if (layer === undefined) {
  ...
  if (layer.canvas) {
    ...
    if (!context || !layer.viewport) return;
    // Turn off image smooth/interpolation if pixelReplication is set in the viewport
