 ...
 * @memberof Internal
 */
export default function (enabledElement: EnabledElement, scale?: number) {
  if (enabledElement.canvas === undefined) {
    throw new Error('enabledElement.canvas must be defined');
  }
  if (enabledElement.image === undefined) {
    throw new Error('enabledElement.image must be defined');
  }
  if (enabledElement.viewport === undefined) {
    throw new Error('enabledElement.viewport must be defined');
  }
  ...
  const angle = enabledElement.viewport.rotation || 0;
  ...
  // Apply the scale
  let widthScale = enabledElement.viewport.scale || 0;
  let heightScale = enabledElement.viewport.scale || 0;
