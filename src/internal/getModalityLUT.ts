 ...
 * @memberof Internal
 */
function generateLinearModalityLUT (slope: number, intercept: number) {
  return (storedPixelValue: number) => storedPixelValue * slope + intercept;
}

function generateNonLinearModalityLUT (modalityLUT: { lut: string | any[]; firstValueMapped: number; }) {
  ...
  return (storedPixelValue: number) => {
 ...
 * @memberof Internal
 */
export default function (slope: number, intercept: number, modalityLUT: any) {
  if (modalityLUT) {
